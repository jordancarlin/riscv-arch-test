...
The available fields are given in framework/src/act/fcov/coverage/RISCV_trace_data.svh.  Most commonly used fields are summarized in <<t-riscvISACOV>>. Other fields include floating-point and vector register IDs and values, MMU extended RVVI signals, vector type information, and pending interrupts. TODO: handling register pairs.
...
|inst_id|instruction_id_t|Instruction identifier|INSTR_ADD
...
[NOTE]
====
Our riscvISACOV implementation generates a structured instruction decoder from fcov/disassemble.svh. The disassembler is used only for optional `FCOV_VERBOSE` diagnostics. When adding new extensions, update the disassembler and instruction sample template.
====
