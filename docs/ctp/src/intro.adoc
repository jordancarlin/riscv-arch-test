...
Coverpoints are written in SystemVerilog, and use the https://github.com/riscv-verification/RVVI[RISC-V Verification Interface] (RVVI). They are independent of the DUT (Device Under Test), so they can be used with any RISC-V implementation. Functional coverage is collected using SystemVerilog infrastructure adapted from https://github.com/riscv-verification/riscvISACOV[riscvISACOV] footnote:[Parts of riscvISACOV that depend on proprietary Synopsys code, such as a disassembler, are replaced with open-source equivalents.].
